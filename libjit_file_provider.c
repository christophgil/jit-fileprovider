...
///
/* forig.c is generated by compile script. Definition of orig_stat() ... */
#include "forig.c"
///

static void jit_file_set_atime(const char *path,const int path_l, struct stat *st){
  if (!path) return;
  long t=time(NULL),t1;
  ...
  if (t-t1>60){ /* Skip if atime newer than .. */
...

    {
      ...
      struct stat st2;
      if (slash && !orig_stat(strcat(strncpy(tmp,path,slash),".mount_info"),&st2)) return;
    }
    struct utimbuf new_times={.actime=t,.modtime=st->st_mtime};
...
#define INIT(orig)  if (atomic_fetch_add(&_inititialized,1)<2) { _init_c(),assert(orig_ ## orig!=NULL),assert(!strcmp(__func__,#orig));}
void _init_c();
...
void _init_c(void){
  ...
  ///
  /* dlsym.c is generated by compile script. */
#include "dlsym.c"
  ///
  ...
  { /* Set variable _hookfile */
    ...
    struct stat st;
    const int res=orig_stat(cg_path_expand_tilde(_hookfile,0,h&&*h?h: DEFAULT_DOT_FILE"/hook.sh"),&st);
    if (res) DIE("Executable hook file %s does not exist: %d %s\n",_hookfile,res, error_symbol(res));
    if (!(st.st_mode&S_IXUSR)) DIE("Hook file %s is not executable\n",_hookfile);
  }
...
/////////////////////////////////////////////////////////////////////////
static int ff_join(char *s,const char **ff, const int n){
  int l=0;
  struct stat st;
  FOR(i,0,n){
    const char *f=ff[i];
    if (!f) continue;
    if (!orig_lstat(f,&st) && st.st_size>0){
      jit_file_set_atime(f,strlen(f),&st);
    }else{
      l+=sprintf(s+l,"%s ",f);
    }
  }
...
static void* hook_thread(void *thread_para){
  ...
  if (ff_join(filelist,ff,n)){
    ...
    if (!pid){
      ...
      //log_debug_now("sssssssssssssssssssssssssssss status %d  %s %s ",status,error_symbol(status), strerror(status));
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
static void hook(const bool is_close, const char *funcname,const char *path){
  //log_entered_function("%s ",path);
  if (!path || *path!='/' || is_close) return;
...
#undef C
  ...
  if (ENDSWITH(path,path_l,".so")) return;
  struct ht unique;  ht_init_with_keystore_dim(&unique,"unique",4,4096);
  const char *funcname_and_ff[HOOK_FILEPATHS+IDX_FILE_LIST+1]={0}; /* NULL terminated list of file paths */
...
#undef E
  ...
  }
  {
    ...
    RLOOP(i,MAX_WAIT_FOR_FILE_SECONDS){
      ...
      bool missing=false;
      RLOOP(k,n0){
        if (!found[k] && (missing=orig_stat(ff[k],&stbuf))){
  ...
  }
  {
    ...
    _sumwait+=ms;
    log_msg(ANSI_FG_GREEN"Time: %'d ms  longest-wait: %'d s sum-wait: %'d s\n"ANSI_RESET,ms,_longestwait/1000,  _sumwait/1000);
    usleep(1024*1024); /* Otherwise Diann says cannot load ... */
  }
}
static void hook_fd(const bool is_close, const char *funcname,const int fd){
  if (fd>2){
    char path[PATH_MAX+1];
    cg_path_for_fd(funcname, path,fd);
    if (!*path) log_error(ANSI_COLOR"path is zero for fd=%d"ANSI_RESET,fd);
    hook(is_close,funcname,path);
    //log_debug_now("fffffffffffffffffffffffffffffffffffffffffffffffffff cg_path_for_fd %d   %s \n",fd, path);
  }
}
///
/* func.c is generated by compile script. */
#include "func.c"
///


int XXXXXXXXmain(int argc,char *argv[]){
  _init_c();
  char tmp[PATH_MAX];
